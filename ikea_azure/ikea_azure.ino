...
{
  sprintf(message, "{\"deviceID\":\"%s\", \"messageID\":%d, \"PPM\":%d, \"AQI\":%d}", IOT_CONFIG_DEVICE_ID, messageID++, C, I);
...
void loop()
{
  ...
  if (mySerial.available()) {
    ...
    int value = mySerial.read();  // Read a character from the software serial port
    //Serial.println(value);            // Print the character to the hardware serial port
    ...
    if(bufferCount == 20) {
      ...
      if (buffer[0] == 22 && buffer[1] == 17 && buffer[2] == 11) {
        C = buffer[6];
